...
function buscarUltimoEstudo(req, res) {
  var idUsuario = req.params.idUsuario;

  estudoModel.buscarUltimoEstudo(idUsuario)
    .then(function (resultado) {
      if (resultado.length > 0) {
...
function buscarDadosGraficoRosca(req, res) {
    var idUsuario = req.params.idUsuario;

    estudoModel.buscarDadosGraficoRosca(idUsuario)
        .then(function (resultado) {
            if (resultado.length > 0) {
                res.status(200).json(resultado);
            } else {
...
function buscarDadosGraficoLinha(req, res) {
    ...
    estudoModel.buscarDadosGraficoLinha(idUsuario)
        .then(function (resultado) {
            if (resultado.length > 0) {
                res.status(200).json(resultado);
            } else {
